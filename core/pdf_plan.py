...
def _analyser_pdf_plan_impl(chemin: str, echelle: int | None = None) -> RapportAnalyse:
    ...
    for niv in tous_niveaux:
        ...
        if info:  # niveau MESURÉ (échelle calibrée sur des cotes)
        ...
        elif surface_annoncee:  # non mesuré -> Superficie TOTALE écrite sur ce plan
            ...
            pieces.append(PieceDetectee(
                ...
                surface_carrelage_m2=s, surface_peinture_murs_m2=0.0, contour=[]))
            lignes.append(MetreLigne(
                poste=f"Surface annoncée — {niv}", type_ouvrage=TypeOuvrage.DALLE,
                quantite=s, unite="m2",
                detail="« Superficie TOTALE » du cartouche (identique sur chaque "
                       "plan de niveau)"))
        else:  # niveau présent, aucune surface disponible
            pieces.append(PieceDetectee(
                id=str(uuid.uuid4())[:8], nom=f"{niv} — surface utile",
    ...
    if tous_niveaux:
        ...
        annonces = [n for n in tous_niveaux if n not in niveaux and surface_annoncee]
        if annonces:
            alertes.append(
                "Surface « annoncée » (Superficie TOTALE du cartouche, "
                f"{surface_annoncee} m²) affichée pour : " + ", ".join(annonces)
                + ". Cette valeur est imprimée à l'identique sur chaque plan de "
                "niveau ; elle n'est pas une mesure séparée par niveau.")

...
- Export **Excel** avec formules + rapport **JSON**
...
Après avoir décompressé le ZIP, ouvrez **CMD** dans le dossier `btp-quant-ai` :

```cmd
cd chemin\vers\btp-quant-ai
...
Dans CMD, toujours dans le dossier `btp-quant-ai` :

```cmd
:: 1. Initialiser le dépôt
git init
git add .
git commit -m "BTP QUANT AI - socle vectoriel DXF"

:: 2. Créer un dépôt vide sur github.com (bouton "New repository"),
::    puis relier votre dossier (remplacez l'URL par la vôtre) :
git remote add origin https://github.com/VOTRE_COMPTE/btp-quant-ai.git
git branch -M main
...
2. **New Project** → **Deploy from GitHub repo** → choisissez `btp-quant-ai`.
...
4. Une fois déployé : onglet **Settings** → **Networking** → **Generate Domain**
   pour obtenir une URL publique (ex. `https://btp-quant-ai.up.railway.app`).
